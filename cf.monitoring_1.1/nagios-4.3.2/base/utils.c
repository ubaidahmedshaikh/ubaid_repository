...
int daemon_init(void) {
	...
	if(fcntl(lockfile, F_SETLK, &lock) < 0) {
		if(errno == EACCES || errno == EAGAIN) {
			...
			logit(NSLOG_RUNTIME_ERROR, TRUE, "Lockfile '%s' looks like its already held by another instance of Real Time Monitoring (PID %d).  Bailing out...", lock_file, (int)lock.l_pid);
			}
		else
...
int query_update_api(void) {
	...
	asprintf(&buf,
	         "POST %s HTTP/1.0\r\nUser-Agent: Real Time Monitoring/%s\r\n"
	         "Connection: close\r\nHost: %s\r\n"
