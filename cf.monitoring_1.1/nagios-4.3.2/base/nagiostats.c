...
#define getopt(argc, argv, OPTSTR) getopt_long(argc, argv, OPTSTR, long_options, &option_index)
	...
	if(error == TRUE || display_help == TRUE) {
		...
		printf("\n");
		printf("Input file:\n");
		printf(" -c, --config=FILE  specifies location of main Real Time Monitoring config file.\n");
		printf(" -s, --statsfile=FILE  specifies alternate location of file to read Real Time Monitoring\n");
		...
		printf(" PROGRUNTIME          string with time Real Time Monitoring process has been running.\n");
		printf(" PROGRUNTIMETT        time Real Time Monitoring process has been running (time_t format).\n");
		...
		printf(" NAGIOSVERSION        string with Real Time Monitoring version.\n");
		printf(" NAGIOSPID            pid number of Real Time Monitoring daemon.\n");
		printf(" NAGIOSVERPID         string with Real Time Monitoring version and PID.\n");
...
static int display_mrtg_values(void) {
	...
	for(temp_ptr = strtok(mrtg_variables, ","); temp_ptr != NULL; temp_ptr = strtok(NULL, ",")) {
		...
		if(!strcmp(temp_ptr, "PROGRUNTIME")) {
			...
			printf("Real Time Monitoring %s (pid=%d)%s", status_version, nagios_pid, mrtg_delimiter);
...
static int display_stats(void) {
	...
	printf("Real Time Monitoring PID:                             %d\n", nagios_pid);
	printf("\n");
