...
#define getopt(argc, argv, o) getopt_long(argc, argv, o, long_options, &option_index)
	...
	if(daemon_mode == FALSE) {
		printf("\nReal Time Monitoring Core %s\n", PROGRAM_VERSION);
		printf("Copyright (c) 2009-present Real Time Monitoring Core Development Team and Community Contributors\n");
	...
	if(error == TRUE || display_help == TRUE) {
		...
		printf("  -d, --daemon                 Starts Real Time Monitoring in daemon mode, instead of as a foreground process\n");
		printf("  -W, --worker /path/to/socket Act as a worker for an already running daemon\n");
		printf("\n");
		printf("Visit the Real Time Monitoring website at https://www.Cyberforza.com/ for bug fixes, new\n");
		printf("releases, online documentation, FAQs, information on subscribing to\n");
		printf("the mailing lists, and commercial support options for Real Time Monitoring.\n");
		printf("\n");
	...
	if(verify_config || test_scheduling || precache_objects) {
		...
		if(result != OK) {
			...
			printf("     version of Real Time Monitoring, you should be aware that some variables/definitions\n");
	...
	/* else start to monitor things... */
		...
		do {
			...
			logit(NSLOG_PROCESS_INFO, TRUE, "Real Time Monitoring %s starting... (PID=%d)\n", PROGRAM_VERSION, (int)getpid());
			...
			if(result != OK)
				logit(NSLOG_PROCESS_INFO | NSLOG_RUNTIME_ERROR | NSLOG_CONFIG_ERROR, TRUE, "Bailing out due to one or more errors encountered in the configuration files. Run Real Time Monitoring from the command line with the -v option to verify your config before restarting. (PID=%d)", (int)getpid());

			else {

				/* run the pre-flight check to make sure everything looks okay*/
				if((result = pre_flight_check()) != OK)
					logit(NSLOG_PROCESS_INFO | NSLOG_RUNTIME_ERROR | NSLOG_VERIFICATION_ERROR, TRUE, "Bailing out due to errors encountered while running the pre-flight check.  Run Real Time Monitoring from the command line with the -v option to verify your config before restarting. (PID=%d)\n", (int)getpid());
				}
