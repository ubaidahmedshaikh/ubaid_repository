...
# Read the contents of the rtm.cfg file.
...
sub scheduleDowntime {
	...
	my $cmdfile = "/usr/local/rtm/var/rw/rtm.cmd";
...
sub checkDowntime {
	...
	while( $eventIndex < @sortedEvents) {
		...
		# Restart Nagios
			...
			system( "/sbin/service rtm start");
			system( "/bin/chgrp rtmcmd /usr/local/rtm/var/rw/rtm.cmd");
...
my $status = `/sbin/service rtm status`;
if( $status !~ /rtm \(pid( \d+)+\) is running.../) {
	print STDERR "Nagios must be running in order to run this test.\n";
	exit 1;
}

# Read the Nagios configuration
my $cfg = readCfgFile( "/usr/local/rtm/etc/rtm.cfg");
