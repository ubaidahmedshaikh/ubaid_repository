...
main(int argc, char **argv) {
	...
	asprintf(&log_filename_localtime, "var/rtm-%02d-%02d-%d-%02d.log", t->tm_mon + 1, t->tm_mday, t->tm_year + 1900, t->tm_hour);
	...
	log_file = "var/rtm.log";
	log_rotation_method = LOG_ROTATION_HOURLY;
	ok(system("cp var/rtm.log.dummy var/rtm.log") == 0, "Copied in dummy rtm.log for archiving");
	ok(rotate_log_file(rotation_time) == OK, "Log rotation should work happily");

	system("diff var/rtm.log var/rtm.log.expected > var/rtm.log.diff");
	ok(system("diff var/rtm.log.diff var/rtm.log.diff.expected > /dev/null") == 0, "Got correct contents of rtm.log");
	unlink("var/rtm.log.diff");

	asprintf(&temp_command, "diff var/rtm.log.dummy %s", log_filename_localtime);
	ok(system(temp_command) == 0, "rtm log archived correctly");

	unlink(log_filename_localtime);
	ok(system("chmod 777 var/rtm.log") == 0, "Changed mode of rtm.log");
	ok(stat("var/rtm.log", &stat_info) == 0, "Got stat info for log file");
	...
	ok(stat("var/rtm.log", &stat_new) == 0, "Got new stat info for new log file");
