...
void display_info_table(const char *title, int refresh, authdata *current_authdata) {
	...
	printf("Real Time Monitoring&trade; %s - <A HREF='https://www.cyberforza.com' TARGET='_new' CLASS='homepageURL'>www.cyberforza.com</A><BR>\n", PROGRAM_VERSION);
...
void include_ssi_files(const char *cgi_name, int type) {
	...
	if(type == SSI_HEADER) {
		printf("\n<!-- Produced by Real Time Monitoring (https://www.cyberforza.com).  Copyright (c) 1999-2007 Ethan Galstad. -->\n");
...
void cgi_config_file_error(const char *config_file) {
	...
	printf("<LI>Make sure you've installed a CGI config file in its proper location.  See the error message about for details on where the CGI is expecting to find the configuration file.  A sample CGI configuration file (named <b>cgi.cfg</b>) can be found in the <b>sample-config/</b> subdirectory of the Real Time Monitoring source code distribution.\n");
	...
	printf("<P>\n");
	printf("Make sure you read the documentation on installing and configuring Real Time Monitoring thoroughly before continuing.  If all else fails, try sending a message to one of the mailing lists.  More information can be found at <a href='https://www.cyberforza.com'>https://www.cyberforza.com</a>.\n");
	printf("</P>\n");
...
void main_config_file_error(const char *config_file) {
	...
	printf("<LI>Make sure you've installed a main config file in its proper location.  See the error message about for details on where the CGI is expecting to find the configuration file.  A sample main configuration file (named <b>rtm.cfg</b>) can be found in the <b>sample-config/</b> subdirectory of the Real Time Monitoring source code distribution.\n");
	...
	printf("<P>\n");
	printf("Make sure you read the documentation on installing and configuring Real Time Monitoring thoroughly before continuing.  If all else fails, try sending a message to one of the mailing lists.  More information can be found at <a href='https://www.cyberfrza.com'>https://www.cyberforza.com</a>.\n");
	printf("</P>\n");
...
void object_data_error(void) {
	...
	printf("<LI>Check the Real Time Monitoring log file for messages relating to startup or status data errors.\n");

	printf("</OL>\n");
	printf("</P>\n");

	printf("<P>\n");
	printf("Make sure you read the documentation on installing, configuring and running Real Time Monitoring thoroughly before continuing.  If all else fails, try sending a message to one of the mailing lists.  More information can be found at <a href='https://www.cyberforza.com'>https://www.cyberforza.com</a>.\n");
	printf("</P>\n");
...
void status_data_error(void) {
	...
	printf("<P>\n");
	printf("The most common cause of this error message (especially for new users), is the fact that Real Time Monitoring is not actually running.  If Real Time Monitoring is indeed not running, this is a normal error message.  It simply indicates that the CGIs could not obtain the current status of hosts and services that are being monitored.  If you've just installed things, make sure you read the documentation on starting Real Time Monitoring.\n");
	printf("</P>\n");
	...
	printf("<LI>Check the Real Time Monitoring log file for messages relating to startup or status data errors.\n");
	printf("<LI>Always verify configuration options using the <b>-v</b> command-line option before starting or restarting Real Time Monitoring!\n");
