...
int main(void) {
	...
	if(display_header == TRUE) {
		...
		display_info_table((log_rotation_method == LOG_ROTATION_NONE || log_archive == 0) ? "Current Event Log" : "Archived Event Log", TRUE, &current_authdata);
		printf("</td>\n");
...
void document_header(int use_stylesheet) {
	...
	printf("<TITLE>\n");
	printf("Real Time Monitoring Log File\n");
	printf("</TITLE>\n");
...
int display_log(void) {
	...
	if(use_lifo == FALSE) {
		...
		if((thefile = mmap_fopen(log_file_to_use)) == NULL) {
			printf("<HR>\n");
			printf("<P><DIV CLASS='errorMessage'>Error: Could not have the log files for reading!</DIV></P>");
			printf("<HR>\n");
			error = TRUE;
			}
		}
