...
int main(void) {
	...
	if(display_header == TRUE) {
		...
		if(display_type == DISPLAY_HOST_INFO)
			...
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "Real Time Monitoring Process Information");
...
void document_header(int use_stylesheet) {
	...
	if (vidurl) {
		...
		printf("var vbox, vBoxId='extinfo%d', vboxText = "
				"'<a href=https://www.cyberforza.com.com/tours target=_blank>"
...
void show_process_info(void) {
	...
	/* PID */
	printf("<TR><TD CLASS='dataVar'>Real Time Monitoring PID</TD><TD CLASS='dataVal'>%d</TD></TR>\n", nagios_pid);
	...
	if(nagios_process_state == STATE_OK) {
		...
		printf("<TR CLASS='command'><TD><img src='%s%s' border=0 ALT='Shutdown the Real Time Monitoring Process' TITLE='Shutdown the Real Time Monitoring Process'></td><td CLASS='command'><a href='%s?cmd_typ=%d'>Shutdown the Real Time Monitoring process</a></td></tr>\n", url_images_path, STOP_ICON, COMMAND_CGI, CMD_SHUTDOWN_PROCESS);
		printf("<TR CLASS='command'><TD><img src='%s%s' border=0 ALT='Restart the Real Time Monitoring Process' TITLE='Restart the Real Time Monitoring Process'></td><td CLASS='command'><a href='%s?cmd_typ=%d'>Restart the Real Time Monitoring process</a></td></tr>\n", url_images_path, RESTART_ICON, COMMAND_CGI, CMD_RESTART_PROCESS);
#endif
		...
		printf("<DIV ALIGN=CENTER CLASS='infoMessage'>It appears as though Real Time Monitoring is not running, so commands are temporarily unavailable...\n");
		printf("</DIV>\n");
		}
...
/*#endif*/
		...
		printf("<DIV ALIGN=CENTER CLASS='infoMessage'>It appears as though Real Time Monitoring is not running, so commands are temporarily unavailable...<br>\n");
		printf("Click <a href='%s?type=%d'>here</a> to view Real Time Monitoring process information</DIV>\n", EXTINFO_CGI, DISPLAY_PROCESS_INFO);
		}
	printf("</TD></TR></TABLE>\n");
...
void show_service_info(void) {
	...
	if(nagios_process_state == STATE_OK &&  is_authorized_for_read_only(&current_authdata) == FALSE) {
		...
		*/
		...
		printf("<DIV CLASS='infoMessage'>It appears as though Real Time Monitoring is not running, so commands are temporarily unavailable...<br>\n");
		printf("Click <a href='%s?type=%d'>here</a> to view Real Time Monitoring process information</DIV>\n", EXTINFO_CGI, DISPLAY_PROCESS_INFO);
		}
