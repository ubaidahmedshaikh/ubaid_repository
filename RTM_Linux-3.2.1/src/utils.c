/****************************************************************************
 *
 * utils.c - RM_LINUX Utility Functions
 *
 * License: GPLv2
 ...
 * Description:
 *
 * This file contains common network functions used in RTM_Linux and check_RTM_Linux.
 *
 * License Notice:
 *
...
int clean_environ(const char *keep_env_vars, const char *RTM_Linux_user)
{
	...
	if (keep_env_vars && *keep_env_vars)
		asprintf(&keep, "%s,RM_LINUX_MULTILINESUPPORT,RM_LINUX_PROGRAMVERSION", keep_env_vars);
	else
		asprintf(&keep, "RM_LINUX_MULTILINESUPPORT,RM_LINUX_PROGRAMVERSION");
	if (keep == NULL) {
	...
	if (RTM_Linux_user != NULL) {
		user = strdup(RTM_Linux_user);
		pw = (struct passwd *)getpwnam(RTM_Linux_user);
	}

	if (RTM_Linux_user == NULL || pw == NULL) {
