/****************************************************************************
 *
 * RTM_Linux.c - Nagios Remote Plugin Executor
 *
 * License: GPLv2
 * Copyright (c) 2009-2017 Nagios Enterprises
 *               1999-2008 Ethan Galstad (nagios@nagios.org)
 *
 * Command line: RTM_Linux -c <config_file> [--inetd | --daemon]
 *
 * Description:
 *
...
char      config_file[MAX_INPUT_BUFFER] = "RTM_Linux.cfg";
...
char     *RTM_Linux_user = NULL;
char     *RTM_Linux_group = NULL;
...
int init(void)
{
	...
	asprintf(&env_string, "RTM_LINUX_MULTILINESUPPORT=1");
	putenv(env_string);
	asprintf(&env_string, "RTM_LINUX_PROGRAMVERSION=%s", PROGRAM_VERSION);
	putenv(env_string);

	/* open a connection to the syslog facility */
	/* facility name may be overridden later */
	get_log_facility(RTM_LINUX_LOG_FACILITY);
	openlog("RTM_Linux", LOG_PID, log_facility);
...
void usage(int result)
{
	...
	printf("RTM_LINUX - Nagios Remote Plugin Executor\n");
	printf("Version: %s\n", PROGRAM_VERSION);
	printf("\n");
	if (result != OK || show_help == TRUE) {
		...
		printf("**      Read the RTM_LINUX SECURITY file for more information     **\n");
		printf("***************************************************************\n");
		printf("\n");
#endif
#ifndef HAVE_LIBWRAP
		...
		printf("\n");
#endif
		printf("Usage: RTM_Linux [-V] [-n] -c <config_file> [-4|-6] <mode>\n");
		printf("\n");
		printf("Options:\n");
		...
		printf("\n");
		printf("Notes:\n");
		printf("This program is designed to process requests from the check_RTM_Linux\n");
		...
		printf("host, RTM_LINUX will execute the command/plugin (as defined in the\n");
		printf("config file) and return the plugin output and return code to the\n");
		printf("check_RTM_Linux plugin.\n");
		printf("\n");
	}
...
void set_stdio_sigs(void)
{
	...
	clean_environ(keep_env_vars, RTM_Linux_user);

	/* drop and then check privileges */
	drop_privileges(RTM_Linux_user, RTM_Linux_group, 0);
	check_privileges();
}
...
int read_config_file(char *filename)
{
	...
	while (fgets(input_buffer, MAX_INPUT_BUFFER - 1, fp)) {
		...
		} else if (!strcmp(varname, "RTM_Linux_user"))
			RTM_Linux_user = strdup(varvalue);

		else if (!strcmp(varname, "RTM_Linux_group"))
			RTM_Linux_group = strdup(varvalue);

		else if (!strcmp(varname, "dont_blame_RTM_Linux"))
...
void conn_check_peer(int sock)
{
	...
	request_init(&req, RQ_DAEMON, "RTM_Linux", RQ_FILE, sock, 0);
	fromhost(&req);
...
# endif
	...
	if (!strcmp(command_name, RTM_LINUX_HELLO_COMMAND)) {
		snprintf(buffer, sizeof(buffer), "RTM_LINUX v%s", PROGRAM_VERSION);
		buffer[sizeof(buffer) - 1] = '\x0';
		...
		if (temp_command == NULL) {
			snprintf(buffer, sizeof(buffer), "RTM_LINUX: Command '%s' not defined", command_name);
			buffer[sizeof(buffer) - 1] = '\x0';
			...
			if (early_timeout == TRUE) {
				sprintf(send_buff, "RTM_LINUX: Command timed out after %d seconds\n",
						command_timeout);
				result = STATE_UNKNOWN;
			} else if (!strcmp(send_buff, "")) {
				sprintf(send_buff, "RTM_LINUX: Unable to read output\n");
				result = STATE_UNKNOWN;
			}
...
int my_system(char *command, int timeout, int *early_timeout, char **output)
{
	...
	*early_timeout = FALSE;		/* initialize return variables */
	...
	if (pid == -1) {
		snprintf(buffer, sizeof(buffer) - 1, "RTM_LINUX: Call to fork() failed\n");
		buffer[sizeof(buffer) - 1] = '\x0';
	...
	if (pid == 0) {
		...
		drop_privileges(RTM_Linux_user, RTM_Linux_group, 1);	/* drop privileges */
		...
		if (fp == NULL) {
			strncpy(buffer, "RTM_LINUX: Call to popen() failed\n", sizeof(buffer) - 1);
			buffer[sizeof(buffer) - 1] = '\x0';
	...
	if ((fd = open(pid_file, O_RDONLY)) >= 0) {
		...
		if (result > 0) {
			...
			if (pid && (pid == getpid() || kill(pid, 0) < 0))
				...
				logit(LOG_ERR, "There's already an RTM_LINUX server running (PID %lu).  Bailing out...", (unsigned long)pid);
				return ERROR;
			}
		}
	}
...
int check_privileges(void)
{
	...
	if (uid == 0 || gid == 0) {
		logit(LOG_ERR, "Error: RTM_LINUX daemon cannot be run as user/group root!");
		exit(STATE_CRITICAL);
	}
