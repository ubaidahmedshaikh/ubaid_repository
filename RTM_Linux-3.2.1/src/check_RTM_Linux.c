/****************************************************************************
 *
 * check_RTM_Linux.c - RTM_LINUX Plugin For Nagios
 *
 * License: GPLv2
 ...
 * Command line: 
 *
 * check_RTM_Linux -H <host_address> [-p port] [-c command] [-to to_sec]
 *
 * Description:
 *
 * This plugin will attempt to connect to the RTM_LINUX daemon on the specified 
...
#define DEFAULT_RTM_LINUX_COMMAND "_RTM_LINUX_CHECK"	/* check version of RTM_LINUX daemon */
...
int packet_ver = RTM_LINUX_PACKET_VERSION_3;
...
int main(int argc, char **argv)
{
	...
	if (result == -1) {
		...
		packet_ver = RTM_LINUX_PACKET_VERSION_2;
	...
	if (result != -1 && force_v2_packet == 0 && packet_ver == RTM_LINUX_PACKET_VERSION_2)
...
int process_arguments(int argc, char **argv, int from_config_file)
{
	...
	while (1) {
		...
		case '2':
			if (from_config_file && packet_ver != RTM_LINUX_PACKET_VERSION_3) {
	...
	if (!from_config_file) {
		snprintf(query, sizeof(query), "%s",
				 (command_name == NULL) ? DEFAULT_RTM_LINUX_COMMAND : command_name);
	...
	if (payload_size > 0 && packet_ver != RTM_LINUX_PACKET_VERSION_2) {
...
void usage(int result)
{
	...
	printf("RTM_LINUX Plugin for Nagios\n");
	printf("Version: %s\n", PROGRAM_VERSION);
	printf("\n");
	...
	if (result != OK || show_help == TRUE) {
		...
		printf("\n");
#endif
		printf("Usage: check_RTM_Linux -H <host> [-2] [-4] [-6] [-n] [-u] [-V] [-l] [-d <dhopt>]\n");
		...
		printf("\n");
		printf("Options:\n");
		printf(" -H, --host=HOST              The address of the host running the RTM_LINUX daemon\n");
		...
		printf("\n");
		printf("Note:\n");
		printf("This plugin requires that you have the RTM_LINUX daemon running on the remote host.\n");
		...
		printf("[command] argument, the RTM_LINUX daemon will run the appropriate plugin command and\n");
...
# endif	/* ifdef SSL_TXT_TLSV1_1 */
		...
		if ((ctx = SSL_CTX_new(meth)) == NULL) {
			printf("CHECK_RTM_LINUX: Error - could not create SSL context.\n");
			exit(STATE_CRITICAL);
		}
...
int connect_to_remote()
{
	...
	if ((ssl = SSL_new(ctx)) == NULL) {
		printf("CHECK_RTM_LINUX: Error - Could not create SSL connection structure.\n");
		return STATE_CRITICAL;
	}
	...
	if ((rc = SSL_connect(ssl)) != 1) {
		...
		if (ssl_err == 5) {
			...
			if (ern == 0)
				printf("CHECK_RTM_LINUX: Error - Could not connect to %s. Check system logs on %s\n", rem_host, rem_host);
			else
				printf("CHECK_RTM_LINUX: Error - Could not connect to %s: %s\n", rem_host, strerror(ern));
		} else {
			printf("CHECK_RTM_LINUX: (ssl_err != 5) Error - Could not complete SSL handshake with %s: %d\n", rem_host, ssl_err);
		}
...
int send_request()
{
	...
	if (packet_ver == RTM_LINUX_PACKET_VERSION_2) {
	...
	if (rc == -1) {
		printf("CHECK_RTM_LINUX: Error sending query to host.\n");
		close(sd);
		return STATE_UNKNOWN;
	}
...
int read_response()
{
	...
	/* recv() error */
	if (rc < 0) {
		if (packet_ver == RTM_LINUX_PACKET_VERSION_3) {
		...
		/* server disconnected */
		printf("CHECK_RTM_LINUX: Received 0 bytes from daemon.  Check the remote server logs for error messages.\n");
		if (packet_ver == RTM_LINUX_PACKET_VERSION_3) {
			if (v3_receive_packet) {
				free(v3_receive_packet);
			}
	...
	if (packet_crc32 != calculated_crc32) {
		printf("CHECK_RTM_LINUX: Response packet had invalid CRC32.\n");
		close(sd);
		if (packet_ver == RTM_LINUX_PACKET_VERSION_3) {
			if (v3_receive_packet) {
				free(v3_receive_packet);
	...
	if (packet_ver == RTM_LINUX_PACKET_VERSION_3) {
		...
		if (v3_receive_packet->buffer_length == 0) {
			printf("CHECK_RTM_LINUX: No output returned from daemon.\n");
		} else {
...
int read_packet(int sock, void *ssl_ptr, v2_packet ** v2_pkt, v3_packet ** v3_pkt)
{
	...
	if (use_ssl == FALSE) {
		...
		if (rc <= 0 || rc != bytes_to_recv) {
			if (rc < bytes_to_recv) {
				if (packet_ver != RTM_LINUX_PACKET_VERSION_3)
					printf("CHECK_RTM_LINUX: Receive header underflow - only %d bytes received (%ld expected).\n", rc, sizeof(bytes_to_recv));
			}
			return -1;
		}

		packet_ver = ntohs(packet.packet_version);
		if (packet_ver != RTM_LINUX_PACKET_VERSION_2 && packet_ver != RTM_LINUX_PACKET_VERSION_3) {
			printf("CHECK_RTM_LINUX: Invalid packet version received from server.\n");
			return -1;
		}

		if (ntohs(packet.packet_type) != RESPONSE_PACKET) {
			printf("CHECK_RTM_LINUX: Invalid packet type received from server.\n");
			return -1;
		}
		...
		if (rc <= 0 || rc != buffer_size) {
			...
			if (rc < buffer_size)
				printf("CHECK_RTM_LINUX: Receive underflow - only %d bytes received (%ld expected).\n", rc, sizeof(buffer_size));
			return -1;
		} else
			tot_bytes += rc;
	}
		...
		if (rc < 0 || bytes_read != buffer_size) {
			...
			if (bytes_read != buffer_size) {
				if (packet_ver == RTM_LINUX_PACKET_VERSION_3) {
					printf("CHECK_RTM_LINUX: Receive buffer size - %ld bytes received (%ld expected).\n", (long)bytes_read, sizeof(buffer_size));
				} else {
					printf("CHECK_RTM_LINUX: Receive underflow - only %ld bytes received (%ld expected).\n", (long)bytes_read, sizeof(buffer_size));
				}
			}
			return -1;
		} else
...
{
	const char	msg1[] = "CHECK_RTM_LINUX STATE ";
