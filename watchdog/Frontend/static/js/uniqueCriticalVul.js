...
$(document).ready(function(){
	var i=0;
	var url = "/server.php?type=external&level=1&category=uniqueCriticalVul";
		...
		$('#table').bootstrapTable({
		    ...
		    columns: [{
		        ...
		        formatter: function ( data, type, row ) 
	        		{
	                    return '<a href="http://web.nvd.nist.gov/view/vuln/detail?vulnId='+data+'" target="_blank">'+ data+'</a>';
	                }
		    }, {
		        field: 'summary',
		        title: 'Description',
...
function download_cves(){
	...
	JSONToCSVConvertor(uniqueVul_data, "Unique Critical Vulnerabilities", true);
}
...
function show_cves(i){
	...
	$('#download_ips').click(function(){
        JSONToCSVConvertor(uniqueVul_data[i-1].ips, "Ips with Unique Critical Vulnerabilities", true);
    });
}
