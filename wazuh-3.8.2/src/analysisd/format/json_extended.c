...
void W_JSON_ParseGroups(cJSON* root, const Eventinfo* lf)
{
    cJSON* groups;
    cJSON* rule;
    int firstPCI, firstCIS, firstGDPR, firstGPG13,firstHIPAA,firstSOX;
    char delim[2];
    char buffer[MAX_STRING] = "";
    char* token;

    firstPCI = firstCIS = firstGDPR = firstGPG13 = firstHIPAA= firstSOX=1;
    delim[0] = ',';
    delim[1] = 0;
    ...
    while(token) {
	 ...
	 else if (add_groupSOX(rule, token, firstSOX)) {
            firstSOX = 0;
        }
...
int add_groupHIPAA(cJSON* rule, char* group, int firstHIPAA)
{
    cJSON* hipaa;
    char *aux;
    // If group begin with HIPAA we have a HIPAA group
    if((startsWith("hipaa_", group)) == 1) {
        // Once we add hipaa group and create array for HIPAA requirements
        if(firstHIPAA == 1) {
        ...
        // Prepare string and add it to HIPAA dss array
        aux = strdup(group);
        str_cut(aux, 0, 6);
...
// Parse groups SOX
int add_groupSOX(cJSON* rule, char* group, int firstSOX)
{
    cJSON* sox;
    char *aux;
    // If group begin with SOX we have a SOX group
    if((startsWith("sox_", group)) == 1) {
        // Once we add sox group and create array for SOX requirements
        if(firstSOX == 1) {
            sox = cJSON_CreateArray();
            cJSON_AddItemToObject(rule, "sox", sox);
        } else {
            sox = cJSON_GetObjectItem(rule, "sox");
        }
        // Prepare string and add it to SOX dss array
        aux = strdup(group);
        str_cut(aux, 0, 4);
        cJSON_AddItemToArray(sox, cJSON_CreateString(aux));
        free(aux);
        return 1;
    }
    return 0;
